...
namespace Cantina_MasterSD.Model
{
    public partial class CancelarPedido : Form
    {
        ...
        private void CancelarPedido_Load(object sender, EventArgs e)
        {
            ...
            {
                if (Dados.usuarioLogado.CpfCnpj == Dados.pedidos[i].Cliente.CpfCnpj)
                {
            ...
            {
                combo_pedidos.Items.Add($"{item.GetQuentinha().Count} Quentinhas - R$ {item.Preco} - {item.Status}");
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            Dados.pedidos[combo_pedidos.SelectedIndex].Status=("Cancelado");
            this.Close();
        }
    }
}
